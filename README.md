...
Collecte : Le collecteur Python interroge l'API GitHub pour récupérer les issues.

Ingestion : Les données sont envoyées vers Kafka (topic: github.issues.raw).

Traitement : Spark Structured Streaming consomme, nettoie et enrichit les données.

Stockage : Les données traitées sont persistées dans MongoDB (upsert).

Prédiction : Les modèles ML prédisent le temps de résolution des issues.

Visualisation : Dashboard Streamlit pour l'analyse et le monitoring.
...
Bash
git clone [https://github.com/your-username/github-issues-prediction-pipeline.git](https://github.com/your-username/github-issues-prediction-pipeline.git)
...
Lancer l'infrastructure (si ce n'est pas déjà fait) :

Bash
docker compose up -d
Démarrer le job Spark Streaming :

Bash
./spark/submit.sh
Lancer le collecteur GitHub :

Bash
docker compose up -d kafka-producer
Accéder au dashboard : Ouvrez http://localhost:8501
...
🔧 Troubleshooting
<details> <summary><b>❌ Kafka ne démarre pas</b></summary>
...
<details> <summary><b>❌ Spark job échoue</b></summary>
...
<details> <summary><b>❌ MongoDB connection refused</b></summary>
...
<details> <summary><b>❌ Dashboard Streamlit ne charge pas</b></summary>
...
👥 Contributeurs
Nom	Rôle
Yahya BAHLOUL	Data Engineer & ML
Zakaria BOUGAYOU	Data Engineer & DevOps
🙏 Remerciements
Apache Spark
...
<p align="center"> <b>⭐ Si ce projet vous aide, n'hésitez pas à lui donner une étoile !</b> </p>